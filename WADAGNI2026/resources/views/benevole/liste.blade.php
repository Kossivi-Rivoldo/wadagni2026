...
<head>
    ...
    <title>Dashboard — Bénévoles</title>
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="main-content">
            <h2 class="mb-4">Liste des bénévoles</h2>
                ...
                @forelse($benevoles as $benevole)
                    <div class="col-md-4">
                        <div class="card shadow-sm h-100">
                            <div class="card-body">
                                <h5 class="card-title">{{ $benevole->nom }}</h5>
                                <p class="mb-1"><strong>Email:</strong> {{ $benevole->email }}</p>
                                <p class="mb-1"><strong>Téléphone:</strong> {{ $benevole->telephone }}</p>
                                <p class="mb-1"><strong>Rôle:</strong> {{ ucfirst($benevole->role) }}</p>
                                <p class="text-muted"><small>Inscrit le: {{ \Carbon\Carbon::parse($benevole->created_at)->format('d/m/Y') }}</small></p>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="col-12">
                        <div class="alert alert-info text-center">Aucun bénévole enregistré pour le moment.</div>
                    </div>
                @endforelse
            </div>
